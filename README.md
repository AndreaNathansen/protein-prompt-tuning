...
In addition to ProtCNN, we also evaluated protein family prediction with [HMMER](http://hmmer.org/) with profile-specific gathering thresholds, which you can run as follows:
```
hmmsearch --cut_ga --tblout <path/to/output/file> <path/to/HMM/file> <path/to/sequences/file>
```
The HMMER runs for all our sets of generated sequences are bundled in the script `hmmer_search.sh`. We use the [Pfam profile HMM downloaded from InterPro](https://www.ebi.ac.uk/interpro/entry/pfam/PF03272/curation/) for the family.
